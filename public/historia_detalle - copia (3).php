...
if ($esAdmin) {
    $res = $conn->query("SELECT c_id, CONCAT(d_apellido, ', ', d_nombre) AS nombre FROM cl_especialista WHERE f_baja IS NULL ORDER BY d_apellido, d_nombre");
    while ($row = $res->fetch_assoc()) { $medicosList[] = $row; }
}
